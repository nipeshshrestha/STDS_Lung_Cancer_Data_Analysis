...
glm1 <- glm(LUNG_CANCER~GENDER+AGE+SMOKING, family=binomial(logit), data=cancer.trainData)
summary(glm1)
glm2 <- glm(LUNG_CANCER~GENDER+AGE+ALCOHOL_CONSUMING, family=binomial(logit), data=cancer.trainData)
summary(glm2)

glm3 <- glm(LUNG_CANCER~GENDER+AGE+SMOKING+ALCOHOL_CONSUMING, family=binomial(logit), data=cancer.trainData)
summary(glm1)
...
# 1st way to find  out the probabiliy that some PEOPLE DID GOT LUNG_cANCER
prob <- 1/(1 + exp(-pred))
head(prob)
#another way to find probability of some people got lung cancer
...
glm2 = glm(LUNG_CANCER ~ SMOKING*GENDER+AGE, family = binomial(logit), data = cancer.trainData)
summary(glm2)

glm3 = glm(LUNG_CANCER ~ ALCOHOL_CONSUMING*GENDER+AGE, family = binomial(logit), data = cancer.trainData)
summary(glm3)
